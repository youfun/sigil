defmodule SigilProbe.HomeScreen.FileNav do
  ...
  def handle({:files, :cancelled}, socket), do: workspace_action(socket, {:files, :cancelled})

  def handle({:files, :picked, items}, socket),
    do: workspace_action(socket, {:files, :picked, items})
