...
extern fn sigil_notify_nif_init() callconv(.c) ?*anyopaque;
extern fn sigil_browser_nif_init() callconv(.c) ?*anyopaque;
...
const base_nifs = [_]ErtsStaticNif{
    ...
    .{ .nif_init = sigil_notify_nif_init, .is_builtin = 0, .nif_mod = THE_NON_VALUE, .entry = null },
    .{ .nif_init = sigil_browser_nif_init, .is_builtin = 0, .nif_mod = THE_NON_VALUE, .entry = null },
};
...
export var erts_static_nif_tab = blk: {
    if (nx_eigen_static and tflite_static) {
